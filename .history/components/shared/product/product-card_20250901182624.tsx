...
const ProductCard = ({
  ...
  const ProductDetails = () => (
    <div className='flex-1 space-y-3 p-3'>
      <p className='font-semibold text-sm text-muted-foreground'>{product.brand}</p>
      <Link
        href={`/product/${product.slug}`}
        className='block overflow-hidden text-ellipsis hover:text-primary transition-colors'
        style={{
          display: '-webkit-box',
          WebkitLineClamp: 2,
      ...
      <div className='flex gap-1 justify-center items-center'>
        ...
        <span className='text-xs text-muted-foreground'>({formatNumber(product.numReviews)})</span>
      </div>
