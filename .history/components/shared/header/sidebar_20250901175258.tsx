...
export default async function Sidebar({
  ...
  return (
    <Drawer direction={getDirection(locale) === 'rtl' ? 'right' : 'left'}>
             <DrawerTrigger className='flex items-center space-x-1 px-2 sm:px-3 py-2 rounded-lg hover:bg-muted/80 transition-colors text-xs sm:text-sm font-medium border border-border/50'>
        <Grid3X3 className='h-4 w-4' />
        <span>{t('Header.Menu')}</span>
      </DrawerTrigger>

      <DrawerContent className='w-full sm:w-[400px] h-[60vh] max-h-[500px] mx-auto rounded-b-xl'>
