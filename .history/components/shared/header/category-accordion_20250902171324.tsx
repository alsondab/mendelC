...
export default function CategoryAccordion({
  ...
  const getCategoryIcon = (categoryName: string) => {
    ...
    if (name.includes('computer') || name.includes('ordinateur'))
      return <Monitor className='h-4 w-4' />
    if (name.includes('video') || name.includes('surveillance'))
      return <Camera className='h-4 w-4' />
    if (name.includes('telephony') || name.includes('téléphonie'))
      return <Phone className='h-4 w-4' />
    if (name.includes('fire') || name.includes('sécurité'))
      return <Shield className='h-4 w-4' />

    // Sous-catégories
    if (name.includes('monitor') || name.includes('écran'))
      return <Monitor className='h-3 w-3' />
    if (name.includes('laptop') || name.includes('portable'))
      return <Laptop className='h-3 w-3' />
    if (name.includes('desktop') || name.includes('bureau'))
      return <Building className='h-3 w-3' />
    if (name.includes('printer') || name.includes('imprimante'))
      return <Printer className='h-3 w-3' />
    if (name.includes('storage') || name.includes('stockage'))
      return <HardDrive className='h-3 w-3' />
    if (name.includes('camera') || name.includes('caméra'))
      return <Camera className='h-3 w-3' />
    if (name.includes('phone') || name.includes('téléphone'))
      return <Phone className='h-3 w-3' />
    if (name.includes('headset') || name.includes('casque'))
      return <Headphones className='h-3 w-3' />
    if (name.includes('dvr') || name.includes('nvr'))
