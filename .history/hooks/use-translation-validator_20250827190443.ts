...
export function useTranslationValidator() {
  ...
  const validateTranslationKey = useCallback((
    ...
    try {
      ...
      const result = t(key)
      return result !== key // Si la traduction existe, elle ne sera pas égale à la clé
    } catch {
      return false
    }
  }, [])
  ...
  const validateCarouselTranslations = useCallback((
    ...
    carousels.forEach((carousel, index) => {
      // Valider le titre
      if (!validateTranslationKey('Home', carousel.title, `Carousel ${index + 1} Title`)) {
        missing.push({
          key: carousel.title,
          namespace: 'Home',
      ...
      // Valider le bouton
      if (!validateTranslationKey('Home', carousel.buttonCaption, `Carousel ${index + 1} Button`)) {
        missing.push({
  ...
  const validateSiteInfoTranslations = useCallback((
    siteInfo: { name: string; slogan: string; description: string }
  ): TranslationValidationResult => {
    const missing: MissingTranslation[] = []
