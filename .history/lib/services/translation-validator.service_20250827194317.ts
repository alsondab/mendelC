...
export class TranslationValidatorService {
  ...
  checkTranslation(
    ...
    const enValue = this.frTranslations[namespace]?.[key]
  ...
  // Obtenir une suggestion de clé propre
  getCleanKeySuggestion(originalKey: string): string {
    return this.cleanKey(originalKey)
  }

  // Obtenir toutes les clés existantes pour un namespace
  getExistingKeys(namespace: string): string[] {
    return Object.keys(this.frTranslations[namespace] || {})
  }
}
