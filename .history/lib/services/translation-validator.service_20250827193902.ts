...
export class TranslationValidatorService {
  ...
  private translationExists(namespace: string, key: string): boolean {
    const frExists = this.frTranslations[namespace] && this.frTranslations[namespace][key]
    const enExists = this.enTranslations[namespace] && this.enTranslations[namespace][key]
    return Boolean(frExists && enExists)
  }

  // Valider les carousels
  validateCarousels(carousels: Array<{ title: string; buttonCaption: string }>): TranslationValidationResult {
  ...
  validateSiteInfo(siteInfo: { name: string; slogan: string; description: string }): TranslationValidationResult {
    const missing: MissingTranslation[] = []
  ...
  checkTranslation(namespace: string, key: string): { exists: boolean; french?: string; english?: string } {
