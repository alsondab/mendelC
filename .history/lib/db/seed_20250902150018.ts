...
const main = async () => {
  try {
    const { users, products, reviews, webPages, settings, categories } = data
    ...
    // Créer un mapping slug -> ObjectId pour les catégories principales
    const categorySlugToId = createdMainCategories.reduce(
      (acc, cat) => {
    ...
    // Créer les sous-catégories avec les ObjectId des parents
    const subCategoriesWithParentIds = subCategories.map((subCat) => ({
      ...subCat,
      parentCategory: categorySlugToId[subCat.parentCategory!],
    }))

    const createdSubCategories = await Category.insertMany(
      subCategoriesWithParentIds
    )
    ...
    // Créer un mapping nom -> ObjectId pour les catégories
    const categoryNameToId = createdCategories.reduce((acc, cat) => {
    ...
    const productsWithCategoryIds = products.map((product) => ({
      ...product,
      _id: undefined,
      category: categoryNameToId[product.category] || null,
      subCategory: product.subCategory ? categoryNameToId[product.subCategory] || null : null,
    }))
