...
// Fonction de formatage des devises avec support pour le Franc CFA
export function formatCurrency(amount: number, currencyCode: string = 'USD', symbol?: string) {
...
// Fonction de formatage des devises avec conversion
export function formatCurrencyWithConversion(
  amount: number, 
  fromCurrency: string = 'USD', 
  toCurrency: string = 'USD',
  currencies: Array<{ code: string; convertRate: number; symbol: string }> = []
) {
  if (fromCurrency === toCurrency) {
    return formatCurrency(amount, toCurrency)
  }
  
  const fromRate = currencies.find(c => c.code === fromCurrency)?.convertRate || 1
  const toRate = currencies.find(c => c.code === toCurrency)?.convertRate || 1
