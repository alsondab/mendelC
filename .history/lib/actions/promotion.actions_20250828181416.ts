...
export const checkAndUpdateExpiredPromotions = async () => {
  try {
    ...
    console.log(
      `🕐 Vérification des promotions expirées à ${now.toISOString()}`
    )
    ...
    for (const product of expiredPromotions) {
      console.log(
        `🔄 Traitement de ${product.name} (expiré le ${product.promotionExpiryDate})`
      )
      await updateProductAfterPromotionExpiry(product)
    }

    console.log(
      `✅ ${expiredPromotions.length} promotions traitées avec succès`
    )
