...
export const checkAndUpdateExpiredPromotions = async () => {
  try {
    ...
    console.log(`🕐 Vérification des promotions expirées à ${now.toISOString()}`)
    ...
    if (expiredPromotions.length === 0) {
      console.log('✅ Aucune promotion expirée à traiter')
      return { success: true, updatedCount: 0 }
    }

    for (const product of expiredPromotions) {
      console.log(`🔄 Traitement de ${product.name} (expiré le ${product.promotionExpiryDate})`)
      await updateProductAfterPromotionExpiry(product)
    }

    console.log(`✅ ${expiredPromotions.length} promotions traitées avec succès`)
...
/**
 * Fonction pour tester localement le système de promotions
 * Appelez cette fonction manuellement en développement
 */
export const testLocalPromotionSystem = async () => {
  try {
    console.log('🧪 Test local du système de promotions...')

    // Simuler une vérification toutes les 2 minutes
    const result = await checkAndUpdateExpiredPromotions()

    console.log('✅ Test local terminé:', result)
    return result
  } catch (error) {
    console.error('❌ Erreur lors du test local:', error)
    return { success: false, error: 'Erreur lors du test local' }
  }
}

const updateProductAfterPromotionExpiry = async (
