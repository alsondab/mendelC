...
const updateProductAfterPromotionExpiry = async (product: any) => {
  try {
    ...
    if (!product.originalTags || product.originalTags.length === 0) {
      product.originalTags = product.tags.filter((tag: string) => tag !== 'todays-deal')
    }
    ...
    console.log(`✅ Promotion expirée pour ${product.name}, nouveaux tags: ${newTags.join(', ')}`)
  } catch (error) {
...
const determineNewTags = async (product: any, currentTags: string[]) => {
  ...
  if (product.numSales > 10) { // Seuil de 10 ventes
    ...
    console.log(`🏆 ${product.name} promu en best-seller (${product.numSales} ventes)`)
  }
 ...
 */
export const activatePromotion = async (productId: string, promotionData: {
  startDate?: Date
  expiryDate: Date
...
export const deactivatePromotion = async (productId: string) => {
  try {
    ...
    const tagsToRestore = product.originalTags && product.originalTags.length > 0 
    ...
    const stats = await Product.aggregate([
      {
        $facet: {
          activePromotions: [
            { $match: { isPromotionActive: true, promotionExpiryDate: { $gt: now } } },
            { $count: 'count' }
          ],
          expiredPromotions: [
            { $match: { isPromotionActive: true, promotionExpiryDate: { $lte: now } } },
            { $count: 'count' }
          ],
          totalPromotions: [
