...
export async function GET(request: NextRequest) {
  try {
    ...
    if (result.success) {
      ...
      console.error('❌ Erreur lors de la vérification des promotions:', result.error)
      return NextResponse.json({
        success: false,
        error: result.error
      }, { status: 500 })
    ...
    console.error('❌ Erreur critique lors de la vérification des promotions:', error)
    return NextResponse.json({
      success: false,
...
export async function POST(request: NextRequest) {
  try {
    console.log('🧪 Test manuel de vérification des promotions...')
    
    const result = await checkAndUpdateExpiredPromotions()
    
    return NextResponse.json({
      success: true,
      message: `Test terminé: ${result.updatedCount} promotions mises à jour`,
      timestamp: new Date().toISOString()
    })
  } catch (error) {
