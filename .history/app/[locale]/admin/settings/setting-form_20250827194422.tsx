...
const SettingForm = ({ setting }: { setting: ISettingInput }) => {
  ...
  React.useEffect(() => {
    if (watchedCarousels && watchedCarousels.length > 0) {
      validateCarouselTranslations(watchedCarousels).catch(console.error)
    }
  }, [watchedCarousels, validateCarouselTranslations])

  React.useEffect(() => {
    if (watchedSiteInfo) {
      validateSiteInfoTranslations(watchedSiteInfo).catch(console.error)
    }
  ...
  const handleAddTranslation = async (
    ...
    try {
      const response = await fetch('/api/translations/add', {
        method: 'POST',
        headers: {
