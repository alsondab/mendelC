    ...
    <div className='space-y-2 sm:space-y-3'>
      {dataWithPercentage.map(
        ({ label, id, value, image, percentage }, index) => (
          <div
            key={`${label}-${index}`}
            className='grid grid-cols-[80px_1fr_60px] sm:grid-cols-[100px_1fr_80px] md:grid-cols-[250px_1fr_80px] gap-1 sm:gap-2'
          >
            {image ? (
              <Link className='flex items-end' href={`/admin/products/${id}`}>
                <Image
                  ...
                  height={28}
                />
                <p className='text-center text-xs sm:text-sm whitespace-nowrap overflow-hidden text-ellipsis'>
                  {label}
                </p>
              </Link>
            ) : (
              <div className='flex items-end text-xs sm:text-sm'>{label}</div>
            )}

            <ProgressBar value={percentage} />

            <div className='text-xs sm:text-sm text-right flex items-center'>
